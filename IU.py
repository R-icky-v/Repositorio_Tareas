...
class frmCrearTarea(tk.Toplevel):
    ...
    def _entry(self, parent, placeholder="", height=None):
        ...
        def on_enter(e): frame.configure(bg=self.C_AZUL)
        def on_leave(e): frame.configure(bg=self.C_GRIS_BD)
        widget.bind("<FocusIn>", lambda e, f=frame: (f.configure(bg=self.C_AZUL), on_focus_in(e)) if hasattr(widget,'_is_ph') else f.configure(bg=self.C_AZUL))
        widget.bind("<FocusOut>", lambda e, f=frame: (f.configure(bg=self.C_GRIS_BD), on_focus_out(e)) if hasattr(widget,'_is_ph') else f.configure(bg=self.C_GRIS_BD))
    ...
    def _build_ui(self):
        ...
        ef2, self.rtbDescripcion = self._entry(body, "Describí los objetivos y requisitos de la tarea...", height=4)
        ef2.pack(fill="x")
    ...
    def _publicar(self):
        ...
        messagebox.showinfo("Tarea publicada", f"'{titulo}' fue publicada correctamente.")
        self.destroy()
